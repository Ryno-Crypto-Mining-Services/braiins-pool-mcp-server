...
export const GetNetworkStatsResponseSchema = z.object({
  ...
  difficulty: z.number().nonnegative().describe('Current network mining difficulty'),
  hashrate_estimate: z.number().nonnegative().describe('Estimated network-wide hashrate in H/s'),
  block_time_target: z
    .number()
    .int()
    .positive()
    .describe('Target block time in seconds (600 for BTC)'),
  block_time_avg: z.number().positive().describe('Average observed block time in seconds'),
